def basePath = 'desktop-common'
folder(basePath)

def prJob = job("$basePath/desktop-common - PR") {
    scm {
        git {
            remote {
                url('https://github.com/wpilibsuite/desktop-common.git')
...
def developmentJob = job("$basePath/desktop-common - Development") {
    triggers {
        scm('H/15 * * * *')
    }
    publishers {
...
def releaseJob = job("$basePath/desktop-common - Release")
...
def setupProperties(job) {
    job.with {
        ...
        properties {
            githubProjectUrl('https://github.com/wpilibsuite/desktop-common')
        }
        logRotator {
            numToKeep(50)
