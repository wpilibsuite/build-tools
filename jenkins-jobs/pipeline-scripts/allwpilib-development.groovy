stage('build') {
    ...
    builds['linux'] = {
        node('linux') {
            ...
            sh './gradlew clean build -PjenkinsBuild -PskipAthena -PreleaseBuild -PbuildAll -PbuildHalStaticDeps --console=plain --stacktrace --refresh-dependencies'
    ...
    builds['windows'] = {
        node('windows') {
            git poll: true, url: 'https://github.com/wpilibsuite/allwpilib.git'
            bat '.\\gradlew.bat  clean build -PjenkinsBuild -PskipAthena -PreleaseBuild -PbuildAll -PbuildHalStaticDeps --console=plain --stacktrace --refresh-dependencies'
    ...
    builds['arm'] = {
        node {
            ws("workspace/${env.JOB_NAME}/arm") {
                ...
                stash includes: '*/libs/**/*.jar, */outputs/**/*.*', name: 'arm'
            }
        }
    }
